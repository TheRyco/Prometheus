...
public class EntityPlayer extends Entity {
	...
	public EntityPlayer(GameScreen screen, GameWorld world) {
		...
		bounds = new Rectangle(position.x, position.y, width / 16.0f * 13.0f,
				height / 16.0f * 15.0f);
		leftBounds = new Rectangle(position.x - 0.1f, position.y, 0.1f,
				height / 16.0f * 15.0f);
		rightBounds = new Rectangle(position.x + (width / 16.0f * 13.0f),
				position.y, 0.1f, height / 16.0f * 15.0f);
		upBounds = new Rectangle(position.x, position.y + height,
				width / 16.0f * 15.0f, 0.1f);
		downBounds = new Rectangle(position.x, position.y - 0.1f,
				width / 16.0f * 15.0f, 0.1f);
	...
	public void render(SpriteBatch sB, ShapeRenderer sR) {
		...
		sR.rect(rightBounds.x, rightBounds.y, rightBounds.width,
				rightBounds.height);
		sR.rect(upBounds.x, upBounds.y, upBounds.width / 16.0f * 13.0f,
				upBounds.height);
		sR.rect(downBounds.x, downBounds.y, downBounds.width / 16.0f * 13.0f,
				downBounds.height);
		...
		if (rightInter != null)
			sR.rect(rightInter.x, rightInter.y, rightInter.width,
					rightInter.height);
		if (upInter != null)
	...
	public void update(float delta) {
		...
		if (screen.inputHandler.keys[Input.Keys.LEFT]
				|| UiHandler.leftButton.isClicked)
			playerState = PlayerState.WALKING_LEFT;
		if (screen.inputHandler.keys[Input.Keys.RIGHT]
				|| UiHandler.rightButton.isClicked)
			playerState = PlayerState.WALKING_RIGHT;
		if (screen.inputHandler.keys[Input.Keys.UP]
				|| UiHandler.upButton.isClicked)
			playerState = PlayerState.WALKING_UP;
		if (screen.inputHandler.keys[Input.Keys.DOWN]
				|| UiHandler.downButton.isClicked)
		...
		System.out.println(collides(0) + "|" + collides(1) + "|" + collides(2)
				+ "|" + collides(3));
	...
	public boolean collides(int dir) {
		...
		if (dir == 0) {
			for (Tile tile : world.currentMap.tiles) {
				if (tile.collidable) {
					if (Intersector.intersectRectangles(leftBounds,
							tile.bounds, leftInter)) {
						return true;
					} else
						leftInter = new Rectangle();
				}
			...
			for (Tile tile : world.currentMap.tiles) {
				if (tile.collidable) {
					if (Intersector.intersectRectangles(rightBounds,
							tile.bounds, rightInter)) {
						return true;
					} else
						rightInter = new Rectangle();
				}
			...
			for (Tile tile : world.currentMap.tiles) {
				if (tile.collidable) {
					if (Intersector.intersectRectangles(upBounds, tile.bounds,
							upInter)) {
						return true;
					} else
						upInter = new Rectangle();
			...
			for (Tile tile : world.currentMap.tiles) {
				if (tile.collidable) {
					if (Intersector.intersectRectangles(downBounds,
							tile.bounds, downInter)) {
						return true;
